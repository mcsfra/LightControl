...
volatile unsigned char bStoreDone = 0;
...
if ( bufCount < 32) {
dbuf[bufCount] = data;
sbuf[bufCount] = spiChecksum;
cbuf[bufCount] = spiCommand;
bufCount++;
}
...
data = getSceneChannelTransparency(help, spiData[1]);
spiChecksum = data;
fifo3Put(data);

data = getChannelValue(spiData[1]);
spiChecksum ^= data;
fifo3Put(data);

data = getSceneChannelLevel(help, spiData[1]);
spiChecksum ^= data;
fifo3Put(data);
...
fifo3Put('!');
bStoreDone = 1;
registerButton(spiData[1],spiData[2]);
}
else {
fifo3Put('?');
}

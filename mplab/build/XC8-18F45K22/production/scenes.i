...
# 11 "scenes.c"
CHANNEL channels[128U];
signed int increment[128U];

# 18
volatile unsigned char dimmingInProgress;

# 23
volatile unsigned char targetScene;
...
return channels[channel].s.level;
}

# 44
unsigned char dimStepsToGo;
...
readSceneChannel(targetScene, i, &sc);

channels[i].s.fraction = 0x00;

f1 = sc.level - channels[i].s.level;

increment[i] = (int) (( f1 * 256.0 ) / (float) ( 80));
...
# 144
void switchSceneOn(unsigned char scene) {
TRISAbits.TRISA0 = 0;
PORTAbits.RA0 = 1;
}

void switchSceneOff(unsigned char scene) {

TRISAbits.TRISA0 = 0;
PORTAbits.RA0 = 0;
}

void continuousDim(unsigned char scene) {
...
unsigned int baseAdress = 0x200;;

baseAdress += ((scene-1) * sizeof(SCENECHANNEL) * 128U) + (channel * sizeof(SCENECHANNEL));
...
HDByteReadI2C( 0xA0, (unsigned char )(baseAdress >> 8) , (unsigned char )(baseAdress & 0xFF) ,(unsigned char*) scp, sizeof(SCENECHANNEL) );
}
...
HDByteWriteI2C( 0xA0, (unsigned char )(baseAdress >> 8) , (unsigned char )(baseAdress & 0xFFU)+1 , scp -> isTransparent);
HDByteWriteI2C( 0xA0, (unsigned char )(baseAdress >> 8) , (unsigned char )(baseAdress & 0xFFU)+2 , scp -> dimSpeed);
}
...
val = (maskArray[i/8] & (1<<(7-(i%8)))) != 0 ? 0 : 1;
HDByteWriteI2C( 0xA0, (unsigned char )(baseAddress >> 8) , (unsigned char )(baseAddress & 0xFF)+1 , val );

# 311
HDByteReadI2C( 0xA0, (unsigned char )(baseAddress >> 8) , (unsigned char )(baseAddress & 0xFF)+1 ,&control, 1 );
...
# 345
void storeCurrentScene(unsigned char scene) {
...
for ( i = 0; i < 128U; i++) {
baseAddress = sceneAdress(scene, i);
HDByteWriteI2C( 0xA0, (unsigned char )(baseAddress >> 8) , (unsigned char )(baseAddress & 0xFF) , channels[i].s.level );
}
}
